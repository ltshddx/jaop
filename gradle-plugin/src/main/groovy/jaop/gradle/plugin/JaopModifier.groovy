...
//    }
    ...
    static void callBefore(Expr m, Config config) {
    ...
    static void bodyBefore(CtBehavior ctBehavior, Config config) {
        def body = "jaop.domain.internal.HookImplForPlugin hook = new jaop.domain.internal.HookImplForPlugin();"
        ...
        body += "new $config.ctMethod.declaringClass.name().$config.ctMethod.name((${config.ctMethod.parameterTypes[0].name})hook);"
        if (config.target.handleSubClass) {
            body = cflow(ctBehavior, body, config)
        }
        ctBehavior.insertBefore(body)
    }

    static void callAfter(Expr m, Config config) {
        def body = '$_ = $proceed($$);'
    ...
    static void bodyAfter(CtBehavior ctBehavior, Config config) {
        def body = "jaop.domain.internal.HookImplForPlugin hook = new jaop.domain.internal.HookImplForPlugin();"
        if (!Modifier.isStatic(ctBehavior.modifiers)) {
            body += "hook.target = \$0;"
        }
...
//        ctMethod.setBody("{$body}", ctMethod.declaringClass.name, ctMethod.name)
    ...
    static String cflow(CtBehavior ctBehavior, String body, Config config) {
        def fname = getCflowField(ctBehavior.declaringClass.classPool, config)
