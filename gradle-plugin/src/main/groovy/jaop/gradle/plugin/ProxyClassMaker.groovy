...
class ProxyClassMaker {
    ...
    static def make(CtMethod ctMethod, Replace replace) {
        ...
        if (makeClass == null) {
            ...
            ctMethod.parameterTypes.eachWithIndex { CtClass entry, int i ->
                ...
                argesParam += "jaop.domain.internal.ObjectWrapUtils.of(param$i),"
            }
            if (paramBody != '') {
            ...
            def returnBodySuffix = ''
            if (ctMethod.returnType != CtClass.voidType) {
                // 基本数据类型在转化object时  dex工具会出错
                returnBodyPrefix = "result = jaop.domain.internal.ObjectWrapUtils.of("
                returnBodySuffix = ')'
            }

            if (Modifier.isStatic(ctMethod.modifiers)) {
                process.setBody("{$returnBodyPrefix $ctMethod.declaringClass.name.$ctMethod.name($paramBody)$returnBodySuffix;return result;}")
            } else {
                process.setBody("{$returnBodyPrefix target.$ctMethod.name($paramBody)$returnBodySuffix; return result;}")
            }
            ...
            println "proxy class for $ctMethod.declaringClass.name.$ctMethod.name has already maked"
        }
